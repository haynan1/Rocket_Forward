...
MAX_NAME_LEN=80
def goal_json(row):
 g=row['goal'];return {'id':g.id,'title':g.title,'description':g.description,'link_url':g.link_url,'date':row['date'].isoformat() if row['date'] else None,'has_deadline':g.has_deadline,'time':g.time.isoformat() if g.time else None,'priority':g.priority,'category':g.category,'status':row['status'],'recurrence_type':g.recurrence_type}
def owned(id):
...
def goals_api():
 ...
 d=request.get_json() or {}; title=str(d.get('title','')).strip(); description=d.get('description'); link_url=d.get('link_url')
 ...
 if not valid_link_url(link_url):return jsonify(error='Link inválido. Use http:// ou https://.'),400
 ...
 has_deadline=bool(d.get('has_deadline',True));g=Goal(user=current_user,title=title,date=due,has_deadline=has_deadline,description=description,link_url=link_url or None,priority=d.get('priority','media'),category=d.get('category','pessoal'));db.session.add(g);db.session.commit();return jsonify(id=g.id),201
...
def goal_api(id):
 ...
 if 'link_url' in d and not valid_link_url(d['link_url']):return jsonify(error='Link inválido. Use http:// ou https://.'),400
 for k in ('title','priority','category'):
  if k in d:setattr(g,k,d[k])
 for k in ('description','link_url'):
  if k in d:setattr(g,k,d[k] or None)
